...
def get_stats(individuals, end=False):
    ...
    """
    initialize_stats()
    stats["random_seed"] = params['RANDOM_SEED']
...
def mean_stats(stats):
    mean_test_fitness = np.mean(np.array([individual.test_fitness for individual in stats]), axis=0)
    std_test_fitness = np.std(np.array([individual.test_fitness for individual in stats]), axis=0)
    best_individual = np.argmax(np.array([individual.test_fitness for individual in stats]), axis=0)
    print("AVERAGE TEST FITNESS: ", mean_test_fitness,
          "\nSTANDARD DEVIATION: ", std_test_fitness,
          "\nBEST INDIVIDUAL: ", stats[best_individual])
